...
use std::env;
    ...
    fn test_pdf_generation_basic() {
        ...
        current_layer.use_text("Hello World", 24.0, Mm(50.0), Mm(200.0), &font);
        let file = std::fs::File::create(&pdf_path).unwrap();
    ...
    fn test_pdf_generation_with_data() {
        let temp_dir = TempDir::new().unwrap();
        let pdf_path = temp_dir.path().join("test_data.pdf");
        let (doc, page1, layer1) = PdfDocument::new(
            "Test PDF with Data",
            Mm(210.0),
            Mm(297.0),
        ...
        let current_layer = doc.get_page(page1).get_layer(layer1);
        let font = doc.add_builtin_font(BuiltinFont::Helvetica).unwrap();
        current_layer.use_text("Test Line 1", 14.0, Mm(30.0), Mm(250.0), &font);
        current_layer.use_text("Test Line 2", 14.0, Mm(30.0), Mm(230.0), &font);
        let file = std::fs::File::create(&pdf_path).unwrap();
        doc.save(&mut BufWriter::new(file)).unwrap();
        assert!(pdf_path.exists());
    }
    ...
    fn test_pdf_generation_landscape() {
        ...
        let current_layer = doc.get_page(page1).get_layer(layer1);
        let font = doc.add_builtin_font(BuiltinFont::Helvetica).unwrap();
        current_layer.use_text("Landscape Page", 18.0, Mm(50.0), Mm(100.0), &font);
        let file = std::fs::File::create(&pdf_path).unwrap();
        doc.save(&mut BufWriter::new(file)).unwrap();
        assert!(pdf_path.exists());
    }
